...
def main(days, limit):
  ...
  messages = MessageQuery(client, columns='*', constraints=['time > now() - {}d'.format(days)], limit=limit)
  mq =  messages.construct_query()
  mq.to_csv("messages.csv")

  feb_stats = FEBStatsQuery(client, columns='*', constraints=['time > now() - {}d'.format(days)], limit=limit)
  fq =  feb_stats.construct_query()
  fq.to_csv("feb_stats.csv")

  columns = [u'daqon', u'datime', u'host', u'msperpoll', u'nclients', u'status']
  drv_stats = DRVStatsQuery(client, columns=columns, constraints=['time > now() - {}d'.format(days)], limit=limit)
  dq =  drv_stats.construct_query()
  dq.to_csv("drv_stats.csv")

  columns = [u'text', u'title']
  events = EventsQuery(client, columns='*', constraints=['time > now() - {}d'.format(days)], limit=limit)
